...
//    private RenderMode renderMode = RenderMode.SQUARE;
    ...
    private MBFImage modelImage, modelImage1, modelImage2, modelImage3, modelImage4, modelImage5;
    ...
    public void StartVideo() throws IOException {
        Video<MBFImage> video = new XuggleVideo(ObjectDetection.class.getResource("data/blood.mkv"));
        int count1 = 0, count2 = 0, count3 = 0, count4 = 0, count5 = 0;
        String o1 = "C:/Users/DR042460/Documents/UMKC/Fall2016/CS5543/ImageClassification/FeatureExtraction/output/features.txt";
        ...
        for (MBFImage mbfImage : video) {
            ...
            System.out.println("Determining matches");
            if (this.matcher1.findMatches(kpl)
                    && ((MatrixTransformProvider) this.matcher1.getModel()).getTransform().cond() < 1e6 ) {
                try {
                    System.out.println("RBC found, determining bounds");
                    final Matrix boundsToPoly = ((MatrixTransformProvider) this.matcher1.getModel()).getTransform()
                            .inverse();
                    System.out.println("Determining if bounds are convex");
                    if (modelImage.getBounds().transform(boundsToPoly).isConvex()) {
                        System.out.println("Convex! drawing shape");
                        renderer.drawShape(this.modelImage.getBounds().transform(boundsToPoly), 3, RGBColour.RED);
                        renderer.drawText("Yeast", 100, 100, HersheyFont.ASTROLOGY, 20, RGBColour.BLACK);
                        System.out.println("Is "+count1+"<= 10?");
                        if(count1 <= 10){
                            System.out.println("Yes!");
                            List<Point2d> vertices = this.modelImage.getBounds().transform(boundsToPoly).asPolygon().getVertices();
                            int x[] = new int[4], y[] = new int[4];
                            for (int i = 0; i < vertices.size(); i++) {
                                x[i] = (int) vertices.get(i).getX();
                                y[i] = (int) vertices.get(i).getY();
                            }
                            System.out.println("Writing RBC features to file");
            ...
            if (this.matcher2.findMatches(kpl)
                    && ((MatrixTransformProvider) this.matcher2.getModel()).getTransform().cond() < 1e10 ) {
                try {
                    System.out.println("WBC found, determining bounds");
                    final Matrix boundsToPoly = ((MatrixTransformProvider) this.matcher2.getModel()).getTransform()
                            .inverse();
                    ;
                    renderer.drawShape(this.modelImage2.getBounds().transform(boundsToPoly), 3, RGBColour.BLUE);
                    renderer.drawText("White Blood Cell", 450, 260, HersheyFont.ASTROLOGY, 50, RGBColour.YELLOW);
                    if(count2 <= 10){
                        System.out.println("Determining if bounds are convex");
                        if (modelImage2.getBounds().transform(boundsToPoly).isConvex()) {
                            ...
                            System.out.println("Writing WBC features to file");
                            Polygon polygon = new Polygon(x, y, 4);
                            for (int i = 0; i < kpl.size(); i++) {
                                if (polygon.contains(kpl.get(i).getX(), kpl.get(i).getY())) {
                                    double c[] = kpl.get(i).getFeatureVector().asDoubleVector();
    ...
    public void LoadReferenceObject() {
        ...
        try {
            modelImage = ImageUtilities.readMBF(ObjectDetection.class.getResource("data/buddingYeast.jpg"));
            //modelImage1 = ImageUtilities.readMBF(ObjectDetection.class.getResource("data/1.jpg"));
            modelImage2 = ImageUtilities.readMBF(ObjectDetection.class.getResource("data/whiteBC.jpg"));
         /*   modelImage3 = ImageUtilities.readMBF(ObjectDetection.class.getResource("data/3.jpg"));
            modelImage4 = ImageUtilities.readMBF(ObjectDetection.class.getResource("data/4.jpg"));
            modelImage5 = ImageUtilities.readMBF(ObjectDetection.class.getResource("data/5.jpg"));
        */} catch (IOException e) {
            e.printStackTrace();
        }
        FImage modelF = Transforms.calculateIntensityNTSC(modelImage);
        this.matcher1.setModelFeatures(engine.findFeatures(modelF));
